...
EXTRA_ENV_VARS="STORAGE_DRIVER=postgres##DB_HOST=/cloudsql/${CLOUDSQL_CONNECTION_NAME:-${GCP_PROJECT_ID}:${REGION}:readon-sql}##DB_PORT=${DB_PORT:-5432}##DB_NAME=${DB_NAME:-images}##DB_USER=${DB_USER:-${READON_CLOUDSQL_TCP_USER:-postgres}}##DB_PASSWORD=${DB_PASSWORD:-${READON_CLOUDSQL_TCP_PASSWORD:-}}##DB_SSL=false##AI_IMAGE_PROVIDER=${AI_IMAGE_PROVIDER:-vertex}##VERTEX_AI_PROJECT=${VERTEX_AI_PROJECT:-${GCP_PROJECT_ID}}##VERTEX_AI_LOCATION=${VERTEX_AI_LOCATION:-${REGION}}##AI_IMAGE_API_KEY=${AI_IMAGE_API_KEY:-}##AI_IMAGE_API_ENDPOINT=${AI_IMAGE_API_ENDPOINT:-https://api.openai.com/v1/images/generations}##AI_IMAGE_MODEL=${AI_IMAGE_MODEL:-gemini-2.5-flash-image}##AI_TIMEOUT=${AI_TIMEOUT:-120000}##CACHE_TTL=${CACHE_TTL:-3600}##RATE_LIMIT_MAX=${RATE_LIMIT_MAX:-100}##RATE_LIMIT_WINDOW=${RATE_LIMIT_WINDOW:-60000}##JSON_BODY_LIMIT=${JSON_BODY_LIMIT:-1mb}##GCP_PROJECT_ID=${GCP_PROJECT_ID}##READON_STORAGE_BUCKET=${READON_STORAGE_BUCKET}"
