...
export async function GET(_request: Request, { params }: { params: { storyId: string } }) {
  ...
  if (row.comprehension_status !== 'ready') {
    return NextResponse.json({ error: 'not_found' }, { status: 404 });
  }
  const resultId = row.comprehension_result_id?.trim();
  if (!resultId) {
    return NextResponse.json({ error: 'not_found' }, { status: 404 });
  }

  const base = comprehensionServiceBase();
  const url = `${base}/comprehension/questions/${encodeURIComponent(resultId)}`;
  const timeoutMs = Number(process.env.READON_COMPREHENSION_FETCH_TIMEOUT_MS ?? 60000);
